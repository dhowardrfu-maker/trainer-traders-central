-- Replace create_order: drop client-supplied tracking_code and qr_payload args.
-- Both are now generated server-side from validated inputs.

DROP FUNCTION IF EXISTS public.create_order(
  uuid, public.carrier, text, integer,
...
BEGIN
  ...
  -- Validate shipping fields server-side (length + minimal shape).
  ...
  -- Server-side tracking code generation by carrier.
  ...
  ELSIF _carrier = 'inpost' THEN
    ...
    v_tracking := 'IN' || body;
  ELSE -- evri
    v_tracking := 'H' || lpad(floor(random() * 9e15 + 1e15)::bigint::text, 16, '0');
  END IF;

  -- Server-built QR payload (uses validated, normalised inputs).
...
REVOKE EXECUTE ON FUNCTION public.create_order(uuid, public.carrier, text, integer, text, text, text, text, text, uuid) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.create_order(uuid, public.carrier, text, integer, text, text, text, text, text, uuid) TO authenticated;