-- Moves the shipping protection decision from the buyer (at checkout) to
-- the seller (when listing/editing the item), where it belongs -- a buyer
-- ticking a box should never be able to commit the seller to a charge they
-- never agreed to.
--
-- Adds listings.shipping_protection_opted_in, set by the seller. create_order
-- reads it directly off the listing row it already looks up (no new
-- parameter needed), so the signature actually goes back to the 14-param
-- version from before the last migration -- the buyer-facing
-- _want_shipping_protection parameter is removed entirely, not just ignored.

ALTER TABLE public.listings
  ADD COLUMN IF NOT EXISTS shipping_protection_opted_in boolean NOT NULL DEFAULT false;
