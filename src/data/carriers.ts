...
const ALL_CARRIERS: CarrierOption[] = [...CARRIERS, ROYAL_MAIL];
 ...
 * Royal Mail code is kept here for when it's re-enabled, even though it's
 * not currently offered to buyers.
 *
 * NOTE: the create-shipping-label edge function keeps its own copy of these
 * codes (with kg= weight bands added) — this export here isn't currently
 * imported by that function, so it's for reference/future use only.
 */
